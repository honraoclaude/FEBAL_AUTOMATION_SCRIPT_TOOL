...
- **D-05 (classification-precision gauge, INFRA-04):** the runtime `classification_precision` gauge = HUMAN-FEEDBACK precision = applied defects / (applied + rejected) among human-REVIEWED drafts (`defects.status`) — the live "are our high-confidence product defects real" signal (the same notion as Phase-9's >90% draft precision). NOT the offline QUAL-03 accuracy (no live ground-truth exists; the classifications table has only predicted class + confidence). Labeled honestly as draft/review precision; zero-reviewed → gauge ABSENT (honest, not 0/NaN-as-real).
- **D-06 (image builds, INFRA-02/03):** the api + web images CI publishes to GHCR / K8s deploys are PRODUCTION multi-stage builds — web = `next build` + `next start` (multi-stage, no dev server/Turbopack); api drops `--reload` (uvicorn workers). "Ships and operates like a product" (SC). The EXISTING dev Dockerfiles stay for local compose; add production stages/targets (or separate prod Dockerfiles) for the published images. (Research already flagged the current web Dockerfile is dev-only and api uses --reload.)

### Claude's Discretion / for research (--research-phase)
- (Q2, planner discretion) Whether the keyless CI lane needs a Postgres `services:` container — inspect the integration-test markers; add `services: postgres` only if the keyless lane touches the DB.
